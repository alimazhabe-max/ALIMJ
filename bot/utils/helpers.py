...
# ───────────────── Inline Keyboard (دکمه‌ها زیر پیام) ─────────────────

def get_main_keyboard(user_id=None):
    """منوی اصلی — زیر همان پیام (مثل قبل)"""
...
def _city_rows(cities, country_code):
    buttons = []
    row = []
    for city in cities:
...
def get_iran_cities_keyboard():
    buttons = _city_rows(IRAN_CITIES, "Iran")
    buttons.append([
...
def get_iraq_cities_keyboard():
    buttons = _city_rows(IRAQ_CITIES, "Iraq")
    buttons.append([
